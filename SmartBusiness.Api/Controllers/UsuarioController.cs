...
[ApiController]
    public class UsuarioController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResult<Token>>> Login([FromBody] AccessCredentials userCredentials, [FromServices] AccessManagerService accessManager)
        {
            ...
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (token != null && token.UserInfo != null)
                    {
                        token.UserInfo.NomeFantasiaEmpresa = _tenant.NomeFantasia;
                        token.UserInfo.RazaoSocialEmpresa = _tenant.RazaoSocial;
                    }
        ...
        public async Task<ActionResult<ApiResult<List<SmartNavigationMenu>>>> ListaItensMenu(int idGrupoUsuario, int idTenant)
        {
            try
            {
                ...
                var itensMenu = PreparaItensMenu2(lstItensMenu); //_mapper.Map<List<VwCtaItensMenuGrupo>, List<ItensMenuGrupo>>(lstItensMenu);
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<ApiResult<List<VwListaUsuarios>>>> ListaUsuarios([FromServices] IBaseRepository<VwListaUsuarios> userRepository)
        {
            ...
            try
            {
                ...
                var lista = userRepository.List(x=>x.IdTenant == this._tenant.IdTenant); 

                return Ok(new ApiResult<List<VwListaUsuarios>>()
                {
                    Success = true,
                    Data = lista.ToList(),
        ...
        private List<SmartNavigationMenu> PreparaItensMenu2(List<VwCtaItensMenuGrupo> itensMenuGrupo)
        {

            var itensMenu = new List<SmartNavigationMenu>();

            var menuNivel1 = itensMenuGrupo.Where(x => x.Nivel == 1).ToList().OrderBy(m => m.Ordem).ToList();

            

            SmartNavigationMenu menu2 = null;

            foreach (var modulo in menuNivel1)
            {
                var menuModulo = new SmartNavigationMenu
                {
                    Key = modulo.IdMenu,
                    ParentKey = modulo.IdMenuPai,
                    TituloMenu = modulo.DescricaoMenu,
                    Area = "",
                    ActionName = "",
                    ControllerName = "",
                ...
                if (!itensMenu.Exists(x => x.Key == menuModulo.Key && x.Nivel == 1 ) )
                {
                    var existChilds = itensMenuGrupo.Where(x => x.IdMenuPai == modulo.IdMenu).Count();

                    if (existChilds > 0)
                    {
                        itensMenu.Add(menuModulo);
                    }
                }

                var menuNivel2 = itensMenuGrupo.Where(x => x.IdMenuPai == modulo.IdMenu && x.Nivel == 2).ToList().OrderBy(m => m.Ordem);

                //Monta os Menu de Nivel 1
                foreach (var item2 in menuNivel2)
                {
                    menu2 = new SmartNavigationMenu
                    {
                        Key = item2.IdMenu,
